...
equity <- read.csv("~/Programming/R/open-advertising-dataset/terms/equity/keyword_stats_20111226_0351841.csv", header=F, stringsAsFactors=F)

# Data Frames with NA values removed
kstats <- suppressWarnings(data[!is.na(as.numeric(data$Global.Monthly.Searches)),])
equity <- suppressWarnings(equity[!is.na(as.numeric(equity$V2)),])

# Number of observations we have of the specific term
observations <- length(row.names(equity))
...
print("IQR value for all observations of equity:")
iqr <- IQR(equity$V2)
iqr
print("P-value for observed IQR of equity:")
...
#print("Std. Deviation value for all observations of equity:")
#sd(equity$V2)
