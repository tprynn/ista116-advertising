...
canon <- read.csv("~/Programming/R/open-advertising-dataset/terms/canon/keyword_stats_20111226_0351841.csv", header=F, stringsAsFactors=F)

# Data Frames with NA values removed
kstats <- suppressWarnings(data[!is.na(as.numeric(data$Global.Monthly.Searches)),])
canon <- suppressWarnings(canon[!is.na(as.numeric(canon$V2)),])

# Number of observations we have of the specific term
observations <- length(row.names(canon))
...
print("IQR value for all observations of canon:")
iqr <- IQR(canon$V2)
iqr
print("P-value for observed IQR of canon:")
...
#print("Std. Deviation value for all observations of canon:")
#sd(canon$V2)
