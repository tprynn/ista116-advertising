...
sim <- read.csv("~/Programming/R/open-advertising-dataset/terms/sim/keyword_stats_20111226_0351841.csv", header=F, stringsAsFactors=F)

# Data Frames with NA values removed
kstats <- suppressWarnings(data[!is.na(as.numeric(data$Global.Monthly.Searches)),])
sim <- suppressWarnings(sim[!is.na(as.numeric(sim$V2)),])

# Number of observations we have of the specific term
observations <- length(row.names(sim))
...
print("IQR value for all observations of sim:")
iqr <- IQR(sim$V2)
iqr
print("P-value for observed IQR of sim:")
...
#print("Std. Deviation value for all observations of sim:")
#sd(sim$V2)
