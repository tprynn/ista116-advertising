...
android <- read.csv("~/Programming/R/open-advertising-dataset/terms/android/keyword_stats_20111226_0351841.csv", header=F, stringsAsFactors=F)

# Data Frames with NA values removed
kstats <- suppressWarnings(data[!is.na(as.numeric(data$Global.Monthly.Searches)),])
android <- suppressWarnings(android[!is.na(as.numeric(android$V2)),])

# Number of observations we have of the specific term
observations <- length(row.names(android))
...
print("IQR value for all observations of android:")
iqr <- IQR(android$V2)
iqr
print("P-value for observed IQR of android:")
...
#print("Std. Deviation value for all observations of android:")
#sd(android$V2)
