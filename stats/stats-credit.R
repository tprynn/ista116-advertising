...
credit <- read.csv("~/Programming/R/open-advertising-dataset/terms/credit/keyword_stats_20111226_0351841.csv", header=F, stringsAsFactors=F)

# Data Frames with NA values removed
kstats <- suppressWarnings(data[!is.na(as.numeric(data$Global.Monthly.Searches)),])
credit <- suppressWarnings(credit[!is.na(as.numeric(credit$V2)),])

# Number of observations we have of the specific term
observations <- length(row.names(credit))
...
print("IQR value for all observations of credit:")
iqr <- IQR(credit$V2)
iqr
print("P-value for observed IQR of credit:")
...
#print("Std. Deviation value for all observations of credit:")
#sd(credit$V2)
