...
insurance <- read.csv("~/Programming/R/open-advertising-dataset/terms/insurance/keyword_stats_20111226_0351841.csv", header=F, stringsAsFactors=F)

# Data Frames with NA values removed
kstats <- suppressWarnings(data[!is.na(as.numeric(data$Global.Monthly.Searches)),])
insurance <- suppressWarnings(insurance[!is.na(as.numeric(insurance$V2)),])

# Number of observations we have of the specific term
observations <- length(row.names(insurance))
...
print("IQR value for all observations of insurance:")
iqr <- IQR(insurance$V2)
iqr
print("P-value for observed IQR of insurance:")
...
#print("Std. Deviation value for all observations of insurance:")
#sd(insurance$V2)
