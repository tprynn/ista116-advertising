...
lemon <- read.csv("~/Programming/R/open-advertising-dataset/terms/lemon/keyword_stats_20111226_0351841.csv", header=F, stringsAsFactors=F)

# Data Frames with NA values removed
kstats <- suppressWarnings(data[!is.na(as.numeric(data$Global.Monthly.Searches)),])
lemon <- suppressWarnings(lemon[!is.na(as.numeric(lemon$V2)),])

# Number of observations we have of the specific term
observations <- length(row.names(lemon))
...
print("IQR value for all observations of lemon:")
iqr <- IQR(lemon$V2)
iqr
print("P-value for observed IQR of lemon:")
...
#print("Std. Deviation value for all observations of lemon:")
#sd(lemon$V2)
